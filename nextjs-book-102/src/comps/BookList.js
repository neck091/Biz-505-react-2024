...
/**
 * BookSearch 에서 검새어를 입력하면
 * search state 에 변화가 발생하고, BookList 에서는
 * 변화된 search state 의 값을 사용하여
 * 도서정보를 Naver 로 부터 fetch 한 후 도서 리스트 state 인
 * books 에 반영하는 일을 수행해야 한다.
 * client 컴포넌트는 BookList 에서 naver_api 모듈의
 * getNaverBooks() 모듈의 함수를 호출하여 naver 로부터 데이터를
 * 가져와야 한다
 * 문제는 naver_api 가 server(use server) 모듈이다
 * client 컴포넌트에서 server 모듈(또는 컴포넌트)의 함수를 호출하여
 * 뭔가 일을 수행하려면 반드시 useEffect() 로 wrapping 해야 한다
 */
const BookList = ({ search, setIsbn }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // client 컴포넌트인 BookList 에서 consile.log()를 사용하면
      // 브라우저의 개발자도구 console 에 내용이 출력된다.
      console.log(result);
      setBooks([...result]);
    };
    fetchData();
